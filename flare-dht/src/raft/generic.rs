...
impl<APP, C> Deref for GenericStateMachine<APP, C>
where
    ...
    APP: AppStateMachine<Req = C::D, Resp = C::R>,
{
    fn load_snapshot(
        ...
        let app = APP::load_snapshot_app(data).map_err(|e| {
    ...
    fn snapshot(&self) -> Result<Vec<u8>, StorageIOError<C::NodeId>> {
        self.app_data
            .snapshot_app()
            .map_err(|e| StorageIOError::read_state_machine(e))
    }

    fn apply_entry(&mut self, entry: Entry<C>) -> C::R {
...
impl<APP, C> std::ops::Deref for LocalStateMachineStore<APP, C>
where
    C: RaftTypeConfig<SnapshotData = Cursor<Vec<u8>>, NodeId: Copy>,
    APP: AppStateMachine<Req = C::D, Resp = C::R>,
{
    type Target = Arc<StateMachineStore<APP, C>>;
    ...
    >,
{
    ...
    async fn install_snapshot(
        ...
        let updated_state_machine =
            GenericStateMachine::load_snapshot(meta, &new_snapshot.data[..])?;
