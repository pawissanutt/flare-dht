...
use super::state_machine::FlareKvResponse;
...
impl<APP: AppStateMachine> RaftSnapshotBuilder<KvTypeConfig> for Arc<StateMachineStore<APP>> {
    #[tracing::instrument(level = "trace", skip(self))]
    async fn build_snapshot(&mut self) -> Result<Snapshot<KvTypeConfig>, StorageError<NodeId>> {
...
impl<APP: AppStateMachine> RaftStateMachine<KvTypeConfig> for Arc<StateMachineStore<APP>> {
    ...
    async fn apply<I>(&mut self, entries: I) -> Result<Vec<FlareKvResponse>, StorageError<NodeId>>
    where
        I: IntoIterator<Item = Entry<KvTypeConfig>> + Send,
    {
        ...
        for entry in entries {
            ...
            match entry.payload {
                EntryPayload::Blank => res.push(FlareKvResponse { value: None }),
                ...
                EntryPayload::Membership(ref mem) => {
                    ...
                    res.push(FlareKvResponse { value: None })
                }
            };
        }
        Ok(res)
    ...
    async fn begin_receiving_snapshot(
        &mut self,
    ) -> Result<Box<<KvTypeConfig as RaftTypeConfig>::SnapshotData>, StorageError<NodeId>> {
    ...
    async fn install_snapshot(
        ...
        snapshot: Box<<KvTypeConfig as RaftTypeConfig>::SnapshotData>,
    ...
    async fn get_current_snapshot(
        &mut self,
    ) -> Result<Option<Snapshot<KvTypeConfig>>, StorageError<NodeId>> {
